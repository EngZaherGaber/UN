...
@Component({
  selector: 'ad-general-transport',
  imports: [DynamicTableComponent, DynmaicFormComponent, DialogModule, CommonModule],
  templateUrl: './ad-general-transport.component.html',
  styleUrl: './ad-general-transport.component.scss'
})
export class AdGeneralTransportComponent {
  info: InfoTable;
  ...
  addFunc: () => void = () => {
    this.resetObjs = {
      general: [
        ...
        {
          key: 'transportCompensation',
          label: 'Transport Compensation',
          value: null,
          dataType: 'int',
          required: true,
          visible: true,
  ...
  ngOnInit() {
    this.info.get$ =
      this.info.getSub$.pipe(
        switchMap((body: any) => {
          if (body) {
            return this.transportSrv.getAll().pipe(
              switchMap(res => {
                return of({
                  data: res.data,
                  columns: [
                    ...
                    {
                      field: 'transportCompensation',
                      header: 'transportCompensation',
                      HeaderType: 'int',
                    },
                    {
                      field: 'createdAt',
  ...
  submit(event: any) {
    ...
    this.transportSrv.add(body).subscribe(res => {
      if (res.success) {
